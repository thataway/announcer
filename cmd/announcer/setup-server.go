...
import (
	...
	"github.com/thataway/announcer/internal/api/announcer"
...
func setupServer(ctx context.Context) (*server.APIServer, error) {
	...
	service, err = announcer.NewAnnounceService(ctx, netInterfaceName)
	if err != nil {
		return nil, err
	}

	var doc *server.SwaggerSpec
	if doc, err = announcer.GetSwaggerDocs(); err != nil {
		return nil, err
	}
